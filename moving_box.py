...
In this project will define a class MovingBox which is used 
to model a moving box and its content. The finished
class must have methods that can be used to add items into
a moving box, remove them,search for a particular item etc.
...
class MovingBox:
    ...
    def __init__(self,name):
        ...
        self.__name = name
        # Attribute is empty dictionary (key: name of item, value: number of item)
        self.__dic = {}

    def add_item(self, item_name, item_count):
        """
        Add item's information into dictionary
        :param item_name: str, name of item
        :param item_count: int, numbers of item
        """

        if item_name not in self.__dic:
            self.__dic[item_name] = item_count
        else:
            self.__dic[item_name] += item_count

    def list_content(self):
        """
        Print the list of each box
        """

        # Create a list of all values of dictionary - number of items
        list_of_number_items = list(self.__dic.values())

        # Calculate the sum of all number of items
        total_items = sum(list_of_number_items)

        # Print
        if self.__dic == {}:
            print('Box "' + f"{self.__name}" + '" contains 0 items.')

        else:
            print('Box "' + f"{self.__name}" + '" contains ' + f'{total_items} items.')

            for item in sorted(self.__dic):
                # Only print the item has number of item is not equal 0
                if self.__dic[item] != 0:
                    print(f"{self.__dic[item]:3.0f} {item}")

    def remove_item(self, item_name, item_count):
        """
        Remove item and its count
        :param item_name: str, name of item to be removed
        :param item_count: int, number of item to be removed
        :return: boolean, True: if remove succeeds, False: if remove fails
        """

        # number of item to be removed must be <= number of item in a source box

        if item_name in self.__dic and item_count <= int(self.__dic[item_name]):
            self.__dic[item_name] -= item_count
            return True
    ...
    def transfer_item(self,target, item_name, item_count):
        """
        Transfer item from a source box to a target box
        :param target: str, a target box to transfer
        :param item_name: str, name of item to be transferred
        :param item_count: str, number of item to be transferred
        :return: boolean, True: if transfer succeeds, False: if transfer fails
        """

        # number of item to be transferred must be <= number of item in a source box

        if item_name in self.__dic and item_count <= int(self.__dic[item_name]):
            self.__dic[item_name] -= item_count

            # If item is not in a target box
            # Then create a new item and its value in a target box

            if item_name not in target.__dic:
                target.__dic[item_name] = item_count

            # If item is already in a target box
            # Sum up number of item into existing number of item
    ...
    def search_item(self, item_name):
        """
        Search Item in the box
        :param item_name: item needed to find
        :return: int, number of item being searched
                      or 0 if we cannot find the item in box
        """

        if item_name in self.__dic:
            item_count = self.__dic[item_name]
